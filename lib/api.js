...
export async function getAllForDiscussion(after) {
  const data = await fetchAPI(
    `
...
export async function getAllSlugs(after) {
  const data = await fetchAPI(
    `
...
export async function migrateAllPosts(after) {
  const data = await fetchAPI(
    `
    query GetAllPost($after: String) {
      posts(first: 100, where: {orderby: {field: DATE, order: DESC}}, after: $after) {
...
export async function getSingleThreadPost(slug) {
  // console.log("query", query);
  const data = await fetchAPI(
    `
    query GeTSinglePost($id: ID!) {
